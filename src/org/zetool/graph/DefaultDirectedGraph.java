...
public class DefaultDirectedGraph implements DirectedGraph, ModifiableGraph {
   ...
   * {@code O(max(initialNodeCapacity, initialEdgeCapacity))}.
   *
   ...
   */
  public DefaultDirectedGraph( int initialNodeCapacity, int initialEdgeCapacity ) {
  ...
  protected DefaultDirectedGraph( DefaultDirectedGraph network ) {
  ...
  /**
   * Returns an {@link DependingListSequence} containing all the edges ending at the specified node. Runtime O(1).
   *
   * @param node the node whose incoming edges are to be returned
   * @return an {@link DependingListSequence} containing all the edges ending at the specified node.
   */
  @Override
  ...
  /**
   * Returns an {@link DependingListSequence} containing all the edges starting at the specified node. Runtime O(1).
   *
   * @param node the node whose outgoing edges are to be returned
   * @return an {@link DependingListSequence} containing all the edges starting at the specified node.
   */
  @Override
  ...
  /**
   * Returns an {@link IdentifiableCollection} containing all the nodes that are incident to an edge ending at the
   * specified node. Runtime O(1).
   *
   * @param node the node which predecessor nodes are to be computed
   * @return an {@link IdentifiableCollection} containing all the nodes that are incident to an edge ending at the
   * specified node.
   */
  @Override
  ...
  /**
   * Returns an {@link IdentifiableCollection} containing all the nodes that are incident to an edge starting at the
   * specified node. Runtime O(1).
   *
   * @param node the node which successor nodes are to be computed
   * @return an {@link IdentifiableCollection} containing all the nodes that are incident to an edge starting at the
   * specified node.
   */
  @Override
  ...
  /**
   * Returns the indegree of the specified node, i.e. the number of edges ending at it. Runtime O(1).
   *
  ...
  /**
   * Returns the outdegree of the specified node, i.e. the number of edges starting at it. Runtime O(1).
   *
  ...
  /**
   * Returns the edge with the specified id or {@code null} if the graph does not contain an edge with the specified id.
   * Runtime O(1).
   *
   * @param id the id of the edge to be returned.
   * @return the edge with the specified id or {@code null} if the graph does not contain an edge with the specified id.
   */
  @Override
  ...
  /**
   * Returns an {@link ListSequence} containing all edges starting at {@code start} and ending at {@code end}. If no
   * such edge exists, an empty list is returned. Runtime O(outdegree(start)).
   *
  ...
  /**
   * Returns the node with the specified id or {@code null} if the graph does not contain a node with the specified id.
   * Runtime O(1).
   *
   * @param id the id of the node to be returned.
   * @return the node with the specified id or {@code null} if the graph does not contain a node with the specified id.
   */
  @Override
  ...
  /**
   * Creates a new directed edge between the specified start and end nodes and adds it to the graph (provided the graph
   * has enough space allocated for an additional edge). Runtime O(1).
   *
  ...
  /**
   * Adds the specified edge to the graph by setting it to it ID's correct position in the internal data structures. The
   * correct position must be empty. Runtime O(1).
   *
  ...
  /**
   * Adds the specified node to the graph by setting it to it ID's correct position in the internal data structures. If
   * this position was occupied before it will be overwritten. Runtime O(1). Note, that the incoming and outgoing edges
   * of any replaced node will be valid for the new one.
   *
  ...
  public DefaultDirectedGraph clone() {
    DefaultDirectedGraph clone = new DefaultDirectedGraph( nodeCount(), edgeCount() );
  ...
  /**
   * Compares the specified object to this object and returns whether the specified object is equivalent to this one. An
   * object is considered equivalent to this network, if and only if it is a network with an equivalent node and edge
   * set. Note that both the node and the edge set must be completely equivalent (i.e. both the visible and hidden parts
   ...
   * this graph. The conversion of nodes and edges to strings is done by the {@code toString} methods of their classes.
   * Runtime O(n + m).
   *
   ...
   * belong to the graph - the only difference to it being actually deleted is that it can be restored very efficiently.
   * This can be useful for residual networks amongst other things. Runtime O(1).
   *

...
/**
 * The term {@code DynamicNetwork} refers to the fact, that the network structure can change over time.
 */
public class DynamicNetwork implements DirectedGraph {
    ...
    public Node opposite(Edge edge, Node node) {
        if (node == edge.start()) {
            ...
            throw new IllegalArgumentException(GraphLocalization.LOC.getString(
                    "ds.graph.NotIncidentException" + node + ", " + edge));
        }
    }
    ...
    public void retainEdges(Collection<Edge> edges) {
        ListSequence<Edge> copy;
        try {
            copy = edges().clone();
        } catch (CloneNotSupportedException ex) {
            throw new IllegalStateException("Could not create copy", ex);
        }
        for (Edge edge : edges) {
            copy.remove(edge);
        }
    ...
    public IdentifiableCollection<Edge> getEdges(Node start, Node end) {
        throw new UnsupportedOperationException(CommonLocalization.LOC.getString(
                "general.NotSupportedException"));
    }

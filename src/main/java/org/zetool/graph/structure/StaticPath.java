 ...
 *
 * If first or last node is given, they are used as first and last nodes. Otherwise, the start of the first edge and the
 * end of the last edge is the last node. This is due to the fact that the path should also work with undirected graphs.
 */
public class StaticPath implements Path {
    ...
    /**
     * Constructs a new path with the given edges. The edges must be consistent, i.e. the endnode of an edge must be
     * equal to the startnode of the next edge (if there follows one more). If the edges are not consistent, an
     ...
     * i.e. i.e. the end node of the current last edge must be equal to the start node of {@code edge}, else an
     * exception is thrown.
     *
    ...
    public final boolean addLastEdge(Edge edge) {
        if (edges.isEmpty() || isConsistent(edges.last(), edge)) {
            edges.add(edge);
            return true;
        } else {
    ...
    /**
     * Extends the path by adding an edge at the start. The edge must be consistent to the current first edge of the
     * path, i.e. i.e. the startnode of the current first edge must be equal to the endnode of {@code edge}.
     *
     * @param edge the edge to insert at the end of the path.
    ...
    public final boolean addFirstEdge(Edge edge) {
        if (edges.isEmpty() || isConsistent(edge, edges.first())) {
            edges.addFirst(edge);
            return true;
        } else {
            return false;
    ...
    /**
     * Returns an iterator for the edges of this path. With the iterator one can iterate comfortable through all the
     * edges of the path.
     *
    ...
    /**
     * Clones this path by cloning the sequence of edges (i.e. all edges are cloned) and creating a new
     * {@code StaticPath} with the clone.
     *
     * @return a {@code StaticPath} object with clones of the edges of this object.
     * @throws java.lang.CloneNotSupportedException
     */
    @Override
    public StaticPath clone() throws CloneNotSupportedException {
        return new StaticPath(edges.clone());
    }

    /**
     * Returns whether an object is equal to this static path. The result is true if and only if the argument is not
     * null and is a {@code StaticPath} object having a sequence of edges that is equal to this path's sequence of edges
     * (i.e. all edges must have the same IDs).
     *
